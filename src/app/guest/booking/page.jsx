...
import { normalizeStopName } from "@/lib/fare";
...
export default function BookingPage() {
    ...
    const loadSchedulesAndStops = async () => {
        try {
            ...
            // If URL contains query params (pickup, drop, date), prefill them
            try {
                if (typeof window !== "undefined") {
                    const params = new URLSearchParams(window.location.search || "");
                    const pu = params.get("pickup") || "";
                    const dr = params.get("drop") || "";
                    ...
                    if (pu) {
                        const nPu = normalizeStopName(pu);
                        puObj = normalizedStops.find((s) => String(s.value) === String(nPu)) || normalizedStops.find((s) => String(s.value).toLowerCase() === String(pu).toLowerCase()) || null;
                        if (puObj) setPickupStop(puObj);
                        else {
                            // fallback: create a synthetic stop object so the dropdown shows selection
                            const synthetic = { value: nPu, name: nPu, label: nPu, marathiName: "" };
                            setPickupStop(synthetic);
                            puObj = synthetic;
                        }
                    }

                    if (dr) {
                        const nDr = normalizeStopName(dr);
                        drObj = normalizedStops.find((s) => String(s.value) === String(nDr)) || normalizedStops.find((s) => String(s.value).toLowerCase() === String(dr).toLowerCase()) || null;
                        if (drObj) setDropStop(drObj);
                        else {
                            const synthetic = { value: nDr, name: nDr, label: nDr, marathiName: "" };
                            setDropStop(synthetic);
                            drObj = synthetic;
                        }
                    }
                    ...
                    // If we have both pickup & drop & date, run search immediately
                    if (puObj && drObj && dt) {
                        // slight delay to ensure state is applied
                        setTimeout(() => {
                            // run silently to avoid showing mandatory-fields toast on initial redirect
                            handleSearchBuses(true);
                        }, 80);
                    }
                }
            } catch (e) {
    ...
    const handleSearchBuses = async (silent = false) => {
        try {
            if (!pickupStop || !dropStop || !travelDate) {
                if (!silent) showAppToast("error", "Pickup, drop and date are mandatory");
                return;
            }
